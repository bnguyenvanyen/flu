...
let rdu = Array.init 12 f 
let s = Array.fold_left (+.) 0. rdu 
...
let main () =
  ...
  scal ~n:12 ~ofsx:1 !init_perturb_r dx0 ;
  let y0 = Vec.make0 24 in
  let y0 = copy ~n:12 ~ofsy:1 ~y:y0 x0 in
  let y0 = copy ~n:12 ~ofsy:13 ~y:y0 dx0 in
  let module Pars = 
    struct
      let size = !size_r
      let r0 = !r0_r
